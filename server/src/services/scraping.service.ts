...
export class ScrapingService {
  ...
  async startScraping(jobId: string): Promise<void> {
    try {
  ...
  // Scrape pricing data
  private async scrapePricing(url: string): Promise<any> {
    ...
    try {
      ...
      const pricingData = await page.evaluate(() => {
        ...
        const planElements = document.querySelectorAll(
          ".pricing-plan, .price-card, .plan, .pricing-table, .pricing-column, [class*='pricing'], [class*='plan']",
        )

        planElements.forEach((element, index) => {
          // Extract plan name
          const nameElement = element.querySelector(
            ".plan-name, .pricing-title, .title, h2, h3, [class*='title'], [class*='name']",
          )
          ...
          const featureElements = element.querySelectorAll(
            ".features li, .plan-features li, .pricing-features li, ul li, [class*='feature'] li",
          )
  ...
  private async scrapeFeatures(url: string): Promise<any> {
    const browser = await puppeteer.launch({
      headless: "new",
      args: ["--no-sandbox", "--disable-setuid-sandbox"],
    })
    ...
    try {
      ...
      const featuresData = await page.evaluate(() => {
        ...
        const featureSections = document.querySelectorAll(
          ".features, .feature-section, [class*='feature'], section, .benefits, [class*='benefit']",
        )
  ...
  private async scrapeIntegrations(url: string): Promise<any> {
    const browser = await puppeteer.launch({
      headless: "new",
      args: ["--no-sandbox", "--disable-setuid-sandbox"],
    })
    ...
    try {
      ...
      const integrationsData = await page.evaluate(() => {
        ...
        const integrationSections = document.querySelectorAll(
          ".integrations, .integration-section, [class*='integration'], .partners, [class*='partner']",
        )
  ...
  private async processScrapedData(job: any, data: any): Promise<void> {
  ...
  private async processPricingData(tool: any, data: any): Promise<void> {
    ...
    for (const [planName, planData] of Object.entries(data)) {
      ...
      const existingPlan = tool.pricingPlans.find((plan: any) => plan.name.toLowerCase() === planName.toLowerCase())
      ...
      if (existingPlan) {
        ...
        for (const feature of typedPlanData.features) {
          ...
          const existingFeature = existingPlan.features.find((f: any) => f.name.toLowerCase() === feature.toLowerCase())
        ...
        await prisma.pricingPlan.create({
          data: {
            ...
            saasToolId: tool.id,
            features: {
              create: typedPlanData.features.map((feature: string) => ({
                name: feature,
              })),
            },
          },
  ...
  private async processFeaturesData(tool: any, data: any): Promise<void> {
  ...
  private async processIntegrationsData(tool: any, data: any): Promise<void> {
    // Process integrations
    for (const integration of data.integrations) {
      // Check if integration already exists
      const existingIntegration = tool.integrations.find((i: any) => i.name.toLowerCase() === integration.toLowerCase())
