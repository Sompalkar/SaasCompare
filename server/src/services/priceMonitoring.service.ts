...
import { sendPriceChangeNotification, sendWeeklyPriceChangeDigest } from './email.service';
import { PricingHistory, PriceChangeLog } from '@prisma/client';
...
interface LatestPricingData extends PricingHistory {
  tool: {
    name: string;
  };
}

interface PricingHistoryWithTool extends PricingHistory {
  tool?: {
    name: string;
  };
}
 ...
 */
export const checkPriceChanges = async (): Promise<void> => {
  try {
    ...
    logger.error('Error checking for price changes:', error);
  }
};
 ...
 */
const getLatestPricingData = async (): Promise<LatestPricingData[]> => {
 ...
 */
const getPreviousPricingData = async (): Promise<PricingHistoryWithTool[]> => {
  // Get pricing history entries that are not marked as latest
  // but are the most recent ones before the latest entries
  const previousEntries = await prisma.$queryRaw<PricingHistoryWithTool[]>`
...
const detectPriceChanges = (
  latestPricing: LatestPricingData[],
  previousPricing: PricingHistoryWithTool[]
): PriceChange[] => {
  const priceChanges: PriceChange[] = [];
  
  // Create a map of previous pricing data for easy lookup
  const previousPricingMap = new Map<string, PricingHistoryWithTool>();
 ...
 */
const recordPriceChanges = async (priceChanges: PriceChange[]): Promise<void> => {
  try {
    // Insert the price changes into a PriceChangeLog table
    await prisma.priceChangeLog.createMany({
    ...
    logger.info(`Recorded ${priceChanges.length} price changes in the database`);
  } catch (error) {
    logger.error('Error recording price changes:', error);
    throw error;
  }
};
 ...
 * Notify users who have subscribed to price change alerts
 */
const notifyUsersOfPriceChanges = async (priceChanges: PriceChange[]): Promise<void> => {
  try {
    ...
    logger.info(`Found ${subscribedUsers.length} users subscribed to price alerts`);
    ...
    logger.error('Error notifying users of price changes:', error);
    throw error;
  }
};
...
const processUserNotifications = async (
  user: { 
    ...
    toolSubscriptions: { toolId: string }[] 
  }, 
  priceChanges: PriceChange[]
): Promise<void> => {
  try {
    // Filter changes to only include tools the user is subscribed to, or all tools if no specific subscriptions
    const relevantChanges = user.toolSubscriptions && user.toolSubscriptions.length > 0
      ? priceChanges.filter(change => 
          user.toolSubscriptions.some((sub) => sub.toolId === change.toolId))
      : priceChanges;
    ...
    const significantChanges = relevantChanges.filter(change => 
      Math.abs(change.changePercentage) >= threshold);
    
    if (significantChanges.length === 0) {
      logger.info(`No significant price changes for user ${user.id}`);
      return;
    }
    
    logger.info(`Sending ${significantChanges.length} price change notifications to user ${user.id}`);
    ...
    logger.error(`Error processing notifications for user ${user.id}:`, error);
  }
};
...
export const sendWeeklyDigests = async (): Promise<void> => {
  try {
    logger.info('Starting weekly price change digest process');
    
    // Get price changes from the last week
    const oneWeekAgo = new Date();
    oneWeekAgo.setDate(oneWeekAgo.getDate() - 7);
    
    const weeklyChanges = await prisma.priceChangeLog.findMany({
      where: {
        timestamp: {
          gte: oneWeekAgo
        }
    ...
    if (weeklyChanges.length === 0) {
      logger.info('No price changes in the last week to include in digest');
      return;
    }
    ...
    logger.info(`Found ${subscribedUsers.length} users subscribed to weekly digests`);
    
    // Process digests for each user
    for (const user of subscribedUsers) {
      // Filter changes to only include tools the user is subscribed to, or all tools if no specific subscriptions
      const relevantChanges = user.toolSubscriptions && user.toolSubscriptions.length > 0
        ? weeklyChanges.filter(change => 
            user.toolSubscriptions.some((sub) => sub.toolId === change.toolId))
        : weeklyChanges;
      
      if (relevantChanges.length === 0) {
        logger.info(`No relevant price changes for user ${user.id} in weekly digest`);
        continue;
      }
      ...
      await sendWeeklyPriceChangeDigest(user.email, formattedChanges);
      
      logger.info(`Sent weekly digest to user ${user.id} with ${formattedChanges.length} price changes`);
    }
  } catch (error) {
    logger.error('Error sending weekly price change digests:', error);
  }
}; 