...
export async function seedTools() {
  ...
  try {
    ...
    if (existingToolsCount > 0) {
      logger.info(`Database already has ${existingToolsCount} tools. Skipping seed.`)
      return
    }
    ...
    for (const tool of tools) {
      ...
      logger.info(`Created tool: ${tool.name} with pricing plans, features, and integrations`)
    }
...
if (require.main === module) {
  seedTools()
    ...
    .catch((e) => logger.error('Error seeding:', e))
} 