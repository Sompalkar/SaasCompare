...
class GeminiAIService {
     ...
     */
    async generateComparisonInsights(comparisonData) {
        try {
            ...
            logger_1.logger.error("Error generating comparison insights:", error);
            throw new Error(`Failed to generate comparison insights: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * Generate personalized recommendations based on user requirements
     */
    async generateRecommendations(userRequirements, comparisonData) {
        try {
     ...
     */
    async generateExecutiveSummary(comparisonData) {
        try {
     ...
     */
    async generatePricingAnalysis(comparisonData) {
        try {
            ...
            logger_1.logger.error("Error generating pricing analysis:", error);
     ...
     */
    async generateFeatureComparison(comparisonData) {
        try {
            ...
            logger_1.logger.error("Error generating feature comparison:", error);
            throw new Error(`Failed to generate feature comparison: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * Generate market positioning analysis
     */
    async generateMarketPositioning(comparisonData) {
        try {
            ...
            logger_1.logger.error("Error generating market positioning:", error);
            throw new Error(`Failed to generate market positioning: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     * Generate future trends analysis
     */
    async generateFutureTrends(comparisonData) {
        try {
            ...
            logger_1.logger.error("Error generating future trends:", error);
     ...
     */
    async generateCompleteReport(comparisonData, userRequirements) {
        try {
            // Generate all sections in parallel for efficiency
            const [executiveSummary, detailedAnalysis, pricingAnalysis, featureComparison, marketPositioning, recommendations, futureTrends,] = await Promise.all([
            ...
            logger_1.logger.error("Error generating complete report:", error);
    ...
    async extractDataFromScreenshot(imageBase64) {
        try {
            const prompt = "Analyze this pricing page screenshot and extract all pricing tiers, features, and limitations in a structured JSON format. Include the following fields for each pricing tier: name, price (as a number or null if it's custom pricing), features (as an array of strings), and limitations (as an array of strings).";
    ...
    buildRecommendationPrompt(userRequirements, comparisonData) {
        return `
    ...
    buildExecutiveSummaryPrompt(comparisonData) {
        return `
    ...
    buildPricingAnalysisPrompt(comparisonData) {
        return `
      Analyze the pricing models of the following SaaS tools:
      ${JSON.stringify(comparisonData.tools.map((t) => ({
            name: t.name,
    ...
    buildFeatureComparisonPrompt(comparisonData) {
        return `
    ...
    buildMarketPositioningPrompt(comparisonData) {
        return `
    ...
    buildFutureTrendsPrompt(comparisonData) {
        return `
      Based on the following SaaS tools and market data, predict future trends in this space:
      ${JSON.stringify(comparisonData.tools.map((t) => t.name), null, 2)}
