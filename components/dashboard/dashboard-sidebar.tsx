...
const SidebarItem = ({ href, icon, title, isActive, isPro = false, onClick }: SidebarItemProps) => {
  return (
    <Link
      href={href}
      className={cn(
...
export default function DashboardSidebar() {
    ...
    <>
      <div className="px-3 py-4">
        <Link href="/" className="flex items-center gap-2 px-2">
          <span className="text-xl font-bold text-purple-600">SaaS Compare</span>
        </Link>
      </div>
      <ScrollArea className="flex-1 px-3">
        <div className="space-y-1">
          ...
          <SidebarItem
            ...
            title="Compare Tools"
            isActive={pathname === "/dashboard/compare" || pathname.startsWith("/dashboard/compare/")}
            onClick={() => setOpen(false)}
          />
          <SidebarItem
        ...
        {isAdmin && (
          <div className="mt-6">
            <h3 className="mb-2 px-4 text-xs font-semibold uppercase text-gray-500">Admin</h3>
            <div className="space-y-1">
              <SidebarItem
  ...
  return <div className="hidden w-64 flex-col border-r bg-white md:flex">{sidebarContent}</div>
}
